...
function App() {
  ...
  useEffect(() => {
    const newColor =
      ...
      customerCount / 5 > partnerCount
        ? "orange"
        : "green";
    setColor(newColor);

    if (curPage === "Meat Market") setColor("green");
    if (curPage === "Sea Food") setColor("green");
    if (curPage === "floral") {
      const newColor =
        (customerCount > 0 && partnerCount === 0) ||
        customerCount / 5 > partnerCount
  ...
  }, [customerCount, partnerCount, curPage, color]);
  ...
  return (
    <div className="App">
      ...
      <Nabvar bcolor={color} onClick={handleViewChange} />
      ...
      {curPage === "floral" && (
        <header className="App-header" style={{ backgroundColor: color }}>
          ...
          />
          <h3>Customers up front: {customerCount}</h3>

          <h3>Partners up front: {partnerCount}</h3>
        </header>
      )}

      {(curPage === "Sea Food" || curPage === "Meat Market") && (
        <header className="App-header" style={{ backgroundColor: "green" }}>
          <h1>{"No Help Needed"}</h1>
          <img
            src={goodLogo}
            className="App-logo"
            alt="logo"
